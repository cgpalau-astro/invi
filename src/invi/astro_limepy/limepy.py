...
import fnc
scipy = fnc.utils.lazy.Import("scipy")
...
class limepy:
    ...
    def __init__(self, phi0, g, **kwargs):
        ...
        self.r0 = 1.0
        if (self.multi):
            self.r0j = sqrt(self.s2j) * self.r0
    ...
    def _set_kwargs(self, phi0, g, **kwargs):
        ...
        if (g < 0):
            raise ValueError("Error: g must be larger or equal to 0")
        if (g >= 3.5):
            raise ValueError("Error: for g>=3.5 models are infinite")

        if (phi0 <= 0):
            raise ValueError("Error: phi0 must be larger than 0")
        ...
        if kwargs is not None:
            ...
            if (self.scale):
                if self._MS is None:
                    ...
                    if (self.verbose):
                        print(
                            " No mass-scale provided, set to default M = 1e5")
                if self._RS is None:
                    self._RS, self.scale_radius = 3, 'rh'
                    if (self.verbose):
                        print(
                            " No radius-scale provided, set to default rh = 3")
                if self._GS is None:
                    self._GS = 0.004302
                    if (self.verbose):
    ...
    def _set_alpha(self):
        ...
        self._set_mass_function_variables()
        self.diff = sum(
            (self._Mjtot / sum(self._Mjtot) - self.Mj / sum(self.Mj))**
            2) / len(self._Mjtot)
        self.niter += 1
        self.nstep = 1
        if (self.verbose):
    ...
    def _poisson(self, potonly):
        ...
        if (not potonly):
            self._y = numpy.r_[self._y, numpy.zeros(2 * self.nmbin)]
        ...
        sol = scipy.integrate.ode(self._odes)
        ...
        if (p <= 0.5):
            ...
            self.rt = rtfac * self.r[-1] if (rtfac
                                             > 1) else 1.0000001 * self.r[-1]
        else:
        ...
        self.rh = scipy.interpolate.BPoly.from_derivatives(self.mc[ih:ih + 2],
        ...
        if (not potonly):
            ...
            if (self.multi):
                for j in range(self.nmbin):
                    ...
                    v2j, v2rj, v2tj = (q * self.s2j[j]
                                       for q in [v2j, v2rj, v2tj])
                    ...
                    mcj = -numpy.r_[self._y[1 + j, :], self._y[1 + j,
                                                               -1]] / self.G
    ...
    def _rhoint(self, phi, r, ra):
        ...
        rhoint = exp(phi) * scipy.special.gammainc(self.g + 1.5, phi)
        ...
        if (self.ra < self.ramax) and (phi > 0) and (r > 0):
            ...
            func = scipy.special.hyp1f1(1, g5, -fp2) if fp2 < self.max_arg_exp else g3 / fp2
            rhoint += p2 * phi**(g + 1.5) * func / scipy.special.gamma(g5)
            rhoint /= (1 + p2)
        return rhoint
    ...
    def _rhov2int(self, phi, r, ra):
        ...
        rhov2r = exp(phi) * scipy.special.gammainc(self.g + 2.5, phi)
        rhov2 = 3 * rhov2r
        rhov2t = 2 * rhov2r
        ...
        if (ra < self.ramax) and (r > 0) and (phi > 0):
            ...
            P1 = p2 * phi**g5 / scipy.special.gamma(g7)
            H1 = scipy.special.hyp1f1(1, g7, -fp2) if fp2 < self.max_arg_exp else g5 / fp2
            H2 = scipy.special.hyp1f1(2, g7,
                        -fp2) if fp2 < self.max_arg_exp else g5 * g3 / fp2**2
    ...
    def _odes(self, x, y, potonly):
        ...
        # y = [phi, u_j, U, K_j], where u = -M(<r)/G
        if (self.multi):
            derivs = [numpy.sum(y[1:1 + self.nmbin]) /
                      x**2] if (x > 0) else [0]
            for j in range(self.nmbin):
                phi = y[0] / self.s2j[j]
                derivs.append(-9.0 * x**2 * self.alpha[j] *
                              self._rhohat(phi, x, j)[0])
        ...
        if (not potonly):  #dK_j/dx
            rhov2j, rhov2rj = [], []
            for j in range(self.nmbin):
                rv2, rv2r, rv2t = self._rhov2int(y[0] / self.s2j[j], x,
                                                 self.raj[j])
                P = self.alpha[j] * self.s2j[
                    j] * 2 * pi * x**2 * rv2 / self.rhoint0[j]
                rhov2j.append(P)

                Pr = self.alpha[j] * self.s2j[
                    j] * 2 * pi * x**2 * rv2r / self.rhoint0[j]
                rhov2rj.append(Pr)
        ...
        dVdvdr = (4 * pi)**2 * x**2 * (2 * y[0])**1.5 / 3 if (x > 0) and (
            y[0] > 0) else 0
    ...
    def legendre_Ul(self, l, theta):
        ...
        dl = 1.0 / 2**l
        m = numpy.arange(il + 1)
        cm = scipy.special.gamma(2 * l - 2 * m + 1) * (-1)**m / (
            scipy.special.gamma(m + 1) * scipy.special.gamma(l - m + 1) * scipy.special.gamma(l - 2 * m + 1))
        gm = cos(theta)**(l - 2 * m)
    ...
    def _rhohat_rot(self):
        for ip in range(len(self.r)):
            for it in range(self.ntheta):
                self.rhon_g[ip][it] = self._rhoint_rot(self.phin_g[ip][it],
                                                       self.r[ip],
                                                       self.theta[it])
        ...
        for il in range(nl):
            ...
            for ir in range(len(self.r)):
                ...
                rho_l[ir][il] = -2.0 * scipy.integrate.simpson(self.rhon_g[ir] * Ul,
                                             x=cos(self.theta))
    ...
    def _rhoint_rot(self, phi, r, theta):
        ...
        # Eq 3
        integ = scipy.integrate.quad(
            lambda x: exp(phi - x) * scipy.special.gammainc(g, phi - x) * numpy.sinh(
                om * Q * x**0.5), 0, phi)[0]

        return integ / (om * Q * scipy.special.gamma(1.5))
    ...
    def _init_rot(self):
        ...
        # Set up grid
        self.r_g, self.theta_g = numpy.meshgrid(self.r,
                                                self.theta,
                                                indexing='ij')
    ...
    def _setup_phi_interpolator(self):
        ...
        self._phi_poly = scipy.interpolate.BPoly.from_derivatives(self.r, phi_and_derivs)
    ...
    def _scale(self):
        ...
        if (self.scale_radius == 'r0'):
            Rstar = self._RS / self.r0
        if (self.scale_radius == 'rh'):
            Rstar = self._RS / self.rh
        if (self.scale_radius == 'rv'):
            Rstar = self._RS / self.rv
        if (self.scale_radius == 'rt'):
            Rstar = self._RS / self.rt
        ...
        # Anisotropy radii
        self.ra, self.raj, self.ramax = (
            q * Rstar for q in [self.ra, self.raj, self.ramax])

        # Scale all variable needed when run with potonly=True
        self.r, self.r0, self.rt = (q * Rstar
                                    for q in [self.rhat, self.r0, self.rt])
        ...
        if (not self.potonly):
            self.rho = self.rhohat * Mstar / Rstar**3
            self.v2, self.v2r, self.v2t = (
                q * v2star for q in [self.v2, self.v2r, self.v2t])
            self.K, self.Kr, self.Kt = (q * Mstar * v2star
                                        for q in [self.K, self.Kr, self.Kt])
            ...
            if (self.multi):
                ...
                self.v2j, self.v2rj, self.v2tj = (
                    q * v2star for q in [self.v2j, self.v2rj, self.v2tj])
    ...
    def _project(self):
        ...
        for i in range(self.nstep - 1):
            ...
            Sigma[i] = 2.0 * abs(scipy.integrate.simpson(self.rho[c], x=z))

            betaterm1 = 1 if i == 0 else 1 - self.beta[c] * R[i]**2 / self.r[
                c]**2

            # eq 41 in paper has a small mistake:  (1-R^2)/r^2 should be (1-R^2/r^2) as below
            betaterm2 = 1 - self.beta[c] if i == 0 else 1 - self.beta[c] * (
                1 - R[i]**2 / self.r[c]**2)

            v2p[i] = abs(2.0 *
                         scipy.integrate.simpson(betaterm1 * self.rho[c] * self.v2r[c], x=z))
            v2p[i] /= Sigma[i]

            v2R[i] = abs(2.0 *
                         scipy.integrate.simpson(betaterm2 * self.rho[c] * self.v2r[c], x=z))
            v2R[i] /= Sigma[i]

            v2T[i] = abs(scipy.integrate.simpson(self.rho[c] * self.v2t[c], x=z))
            v2T[i] /= Sigma[i]
            ...
            if (i > 0):
                x = self.r[i - 1:i + 1]
                mcp[i] = mcp[i -
                             1] + 2 * pi * scipy.integrate.simpson(x * Sigma[i - 1:i + 1], x=x)

            if (self.multi):
                for j in range(self.nmbin):
                    Sigmaj[j, i] = 2.0 * scipy.integrate.simpson(self.rhoj[j, c], x=z)
                    if (i > 0):
                        x = self.r[i - 1:i + 1]
                        mcpj[j, i] = mcpj[j, i - 1] + 2 * pi * scipy.integrate.simpson(
                            x * Sigmaj[j, i - 1:i + 1], x=x)

                    if (i == 0):
                        betaterm1 = 1
                        betaterm2 = 1 - self.betaj[j, c]
                    else:
                        betaterm1 = 1 - self.betaj[j,
                                                   c] * R[i]**2 / self.r[c]**2
                        # eq 41 in paper has a small mistake:  (1-R^2)/r^2 should be (1-R^2/r^2) as below
                        betaterm2 = 1 - self.betaj[j, c] * (
                            1 - R[i]**2 / self.r[c]**2)

                    v2int = scipy.integrate.simpson(betaterm1 * self.rhoj[j, c] *
                                  self.v2rj[j, c],
                                  x=z)
                    v2pj[j, i] = abs(2.0 * v2int)
                    v2pj[j, i] /= Sigmaj[j, i]

                    v2int = scipy.integrate.simpson(betaterm2 * self.rhoj[j, c] *
                                  self.v2rj[j, c],
                                  x=z)
                    ...
                    v2Tj[j,
                         i] = abs(scipy.integrate.simpson(self.rhoj[j, c] * self.v2tj[j, c],
                                        x=z))
                    v2Tj[j, i] /= Sigmaj[j, i]

        # Radius containing half the mass in projection
        x = self.r[-2:]
        mcp[-1] = mcp[-2] + 2 * pi * scipy.integrate.simpson(x * Sigma[-2:], x=x)
        ...
        if (self.multi):
            ...
            for j in range(self.nmbin):
                mcpj[j,
                     -1] = mcpj[j,
                                -2] + 2 * pi * scipy.integrate.simpson(x * Sigmaj[j, -2:], x=x)
    ...
    def get_Paz(self, az_data, R_data, jns):
        ...
        if (R_data < self.rt):
            ...
            spl_Mr = scipy.interpolate.UnivariateSpline(self.r, self.mc, s=0,
            ...
            az_spl = scipy.interpolate.UnivariateSpline(
                z, az, k=4, s=0,
            ...
            rho_spl = scipy.interpolate.UnivariateSpline(self.r, rho, ext=1, s=0)
            rhoz = rho_spl(sqrt(z**2 + R_data**2))
            rhoz_spl = scipy.interpolate.UnivariateSpline(z, rhoz, ext=1, s=0)
            ...
            if len(zmax) > 0:
                ...
                z1_spl = scipy.interpolate.UnivariateSpline(az_spl(z1), z1, k=k, s=0, ext=1)
                z2_spl = scipy.interpolate.UnivariateSpline(az_spl(z2), z2, k=k, s=0, ext=1)
    ...
    def df(self, *arg):
        ...
        if (len(arg) < 2) or (len(arg) == 5) or (len(arg) == 6) or (len(arg)
                                                                    > 7):
        ...
        if (sum(c) > 0):
            ...
            if (self.g > 0):
                DF[c] *= scipy.special.gammainc(self.g, E[c])

            if (len(arg) == 7):
                J2 = v2 * r2 - (x * vx + y * vy + z * vz)**2
            if (len(arg) == 4):
                J2 = sin(theta)**2 * v2 * r2
            if (len(arg) <= 3):
                J2 = numpy.zeros(len(c))
