...
class sample:
    ...
    def _set_params_and_init(self, mod, **kwargs):
        if not mod.converged:
            raise ValueError(
                " Error: This model has not converged, abort sampling ...")
        ...
        if (mod.multi):
            ...
            if min(self.Nj) == 0:
                raise ValueError(
                    " Error: One of the mass components has zero stars!")
        self.mod = mod
    ...
    def _sample_r(self, m):
        ...
        if m.multi:
            ...
            for j in range(m.nmbin):
                ...
                self.r[s:e] = numpy.interp(ran[s:e], m.mcj[j] / m.mcj[j][-1],
                                           m.r)
    ...
    def _sample_v(self, m):
        ...
        for j in range(self.mod.nmbin):
            if (self.verbose):
                print("   set-up segments for velocity cdf ...")

            self.ycum = numpy.zeros(self.N)
            self.y = self._pdf_k32(self.r, self.phihat, self.x[0], j)
            for i in range(1, nx):
                self.y = numpy.vstack(
                    (self.y, self._pdf_k32(self.r, self.phihat, self.x[i], j)))
            ...
            if (self.verbose):
                print("   (rejection) sampling of k^3/2 values ...")
            self._sample_k(j)
    ...
    def _Eg(self, x, s):
        return exp(x) * scipy.special.gammainc(s, x) if s > 0 else exp(x)
    ...
    def _pdf_k32(self, r, f, x, j):
        ...
        if (sum(c) > 0):
            if (self.ani):
                p = r / self.mod.raj[j]
                F = scipy.special.dawsn(x13 * p * sqrt(sig2fac)) / p
    ...
    def _pdf_angle(self, q, *args):
        ...
        # P(q) = scipy.special.erfi(sqrt(k)*p*q)/scipy.special.erfi(sqrt(k)*p)

        a, R = args
        return R - scipy.special.erfi(a * q) / scipy.special.erfi(a)
    ...
    def _sample_angles(self, N):
        ...
        if self.ani:
            # Anisotropic systems sample angles: cdf(q) = scipy.special.erfi(a*q)/scipy.special.erfi(a), a = sqrt(k)*p
            p = self.r / self.ra
            a = p * sqrt(self.k)
            if self.mod.multi:
            ...
            for j in range(N):
                self.q[j] = scipy.optimize.brentq(self._pdf_angle,
                                            0,
                                            1,
                                            args=(a[j], R[j]))
        else:
    ...
    def _summary(self):
        ...
        print("       U: sample = %12.4e; model = %12.4e" %
              (0.5 * sum(self.m * f), m.U))
        print("       K: sample = %12.4e; model = %12.4e" %
              (sum(0.5 * self.m * self.v**2), m.K))
        print("       Q: sample = %12.4e; model = %12.4e" %
              (sum(0.5 * self.m * self.v**2) /
               (0.5 * sum(self.m * f)), m.K / m.U))
        print("  2Kr/Kt: sample = %12.4f; model = %12.4f" %
              (2 * sum(self.vr**2) / sum(self.vt**2), 2 * m.Kr / m.Kt))
        ...
        if m.multi:
            ...
            for j in range(m.nmbin):
                c = (self.m == m.mj[j])
                print("      Kj: sample = %12.4e; model = %12.4e" %
                      (sum(0.5 * self.m[c] * self.v[c]**2), m.Kj[j]))

            for j in range(m.nmbin):
                c = (self.m == m.mj[j])
                print(" 2Kr/Ktj: sample = %12.4e; model = %12.4e" %
                      (2 * sum(self.vr[c]**2) / sum(self.vt[c]**2),
                       2 * m.Krj[j] / m.Ktj[j]))
