...
Note
----
1)  Python does not copy when defining a dictionary. For example:
    x = np.zeros(10)
    d = {'x': x}
    x[3] = 5.0
    print(d['x'])
...
def dict_to_array(xdict):
    ...
    plxICRS = ['parallax', 'delta', 'alpha', 'mu_r', 'mu_delta', 'mu_alpha_str']
    ...
    alpha = ['a_r', 'a_phi', 'a_z']
    ALPHA = ['A_1', 'A_2', 'A_3']
    phi = ['r', 'phi_2', 'phi_1', 'mu_r', 'mu_phi_2', 'mu_phi_1_str']

    for item in [icrs, plxICRS, car, cyl, sph, aaf, AAF, alpha, ALPHA, phi]:
...
def array_to_dict(arr, type_keys):
    ...
    match type_keys:
        ...
        case 'plxICRS':
            keys = ['parallax', 'delta', 'alpha', 'mu_r', 'mu_delta', 'mu_alpha_str']
        case 'car':
        ...
        case 'ALPHA':
            keys = ['A_1', 'A_2', 'A_3']
        case 'phi':
            keys = ['r', 'phi_2', 'phi_1', 'mu_r', 'mu_phi_2', 'mu_phi_1_str']
        case _:
            raise KeyError("Allowed type_keys: {'ICRS', 'plxICRS', 'car', 'cyl', 'sph', 'aaf', 'dgc', 'AAF', 'alpha', 'ALPHA', 'phi'}")
...
def _phi(w_icrs, prm_gc):
    J = _np.array(prm_gc['stream']['phi']['J_flat'])
    w_phi = _invi.coordinates.ICRS_to_phi.matrix(w_icrs, J)
    w_phi_dict = _invi.dicts.array_to_dict(w_phi, 'phi')
    return w_phi_dict


def phase_space(w_fsr, w_icrs, prm_gc):
    return {'FSR': _FSR(w_fsr), 'ICRS': _ICRS(w_icrs), 'phi': _phi(w_icrs, prm_gc)}
