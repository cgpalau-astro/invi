...
def _determine_number_stars(mass, mass_sample, rounding='superior'):
    ...
    if _np.sum(mass_sample) < mass:
        raise ValueError(f"Requested mass < mass\n Maximum allowed mass = {_np.sum(mass_sample)} [M_sun]")
    #------------------------------------------------------
...
def load(n_stars, cmd_file, shuffle=False, random_state=None, verbose=True):
    ...
    n_tot = len(cmd['mass'])
    if n_stars > n_tot:
        raise ValueError(f"n_stars must be smaller than the total number of stars in the cmd file: n_stars = {n_stars:_} > {n_tot:_} = n_tot")
