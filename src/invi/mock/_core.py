...
def _dr3_rv(dec, ra, g, rp):
    ...
    coordinates = _astropy.coordinates.SkyCoord(ra=ra, dec=dec, frame='icrs', unit='deg')
    probability = rvssf.query(coordinates, g=g, c=g-rp)
    return probability
...
def selection(sample_parallax_dict, sample_icrs_dict, mock_sf, s_unc, s_phot, prm_gc, prm, progress=True):
    #Create file mocking gaia catalogue with extracted columns.
    ...
    file_orbit_bundle = "/home/carles/projects/invi/data/gaia_dr3/M68/selection/orbit_bundle.dat"
    ...
    r_orb_est = _invi.stars.orbit_estimate.distance(ra, dec, prm_gc, prm, progress=progress)
...
def uncertainties(gdr, s_icrs_dict, synthetic_population, s_phot, mock_red):
    ...
    Note
    ----
    1)  The uncertainties are computed for the g_red"""
    #---------------------------------------------
    def within_limits(x):
