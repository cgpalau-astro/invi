...
__all__ = ["find_peak_freqs", "dominant_peak"]
...
def find_peak_freqs(f_k, t, n_freqs=1, p=1, spec=False,
    ...
    om = 2.*np.pi*np.fft.fftfreq(N, T/N)
    ...
    for k in range(n_freqs):
        ...
        if (mn_phi_om_inf > mn_phi_om_max) & (mn_phi_om_sup > mn_phi_om_max):
            best_peak = scipy.optimize.minimize_scalar(mn_phi_om, args=(f_k_chi, t_sym),
            ...
            print('Frequency ', k+1, ' - Peak not found in first shot. Refining...')
            try:
                ...
                print('Frequency ', k+1, 'Unable to find peak frequency')
                break
...
def dominant_peak(x, t):
    """Compute frequency of the dominant peak using naif.
    ...
    Doc
    ---
    1)  https://naif.readthedocs.io/en/latest/index.html
