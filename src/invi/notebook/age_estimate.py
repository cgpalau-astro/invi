...
def plot_radec(name_folder, T, prm_gc, s=0.5):
    ...
    n_samples = _invi.stream.age_estimation._count_files(name_folder, 'sample_[0-9]*')

    fig, ax = _fnc.plot.figure(fc=1.75)

    #Plot all stars with sky coordinates
    for i in range(n_samples):
        name_file = f"{name_folder}/sample_{i}.npz"
        sim_AAF = _np.load(name_file)['sim_AAF']
        sim_icrs = _np.load(name_file)['sim_icrs']
        magnitude_cut = _np.load(name_file)['magnitude_cut']
        ra = sim_icrs[2] #[deg]
        dec = sim_icrs[1] #[deg]
        sim_stripping_time = -_invi.inverse.integration_time(sim_AAF)
        selected = sim_stripping_time[magnitude_cut] > -T

        ax.scatter(ra[selected], dec[selected], s=s, c='darkblue')
    ...
    for i in range(n_samples):
        ...
        ax.scatter(ra[selected], dec[selected], s=s, c='r')
    ...
    ax.set_title(f"T = {T:_} Myr, n_samples={n_samples:_}, n_stars={len(ra):_}")
...
def hist_density(name_folder, T, A1, prm_gc, color='r', y_limit=6.0):
    ...
    data = _invi.stream.age_estimation.load_samples(name_folder)
    A1_sim = _invi.stream.age_estimation.surface_density(data, T)
...
def hist_area(name_folder, T, A1, prm_gc, color='r'):
    ...
    data = _invi.stream.age_estimation.load_samples(name_folder)
    A1_sim = _invi.stream.age_estimation.surface_density(data, T)
    n_stars = data = _invi.stream.age_estimation.number_stars(name_folder, T)
    mean_stars = _np.mean(n_stars)
    std_stars = _np.std(n_stars)

    _fig, ax = _fnc.plot.figure(fc=(2.5,1.0))

    kwargs = {'bins': bins, 'range': rng}

    n_stars = len(A1)
    area = n_stars*(rng[1] - rng[0])/bins
    _fnc.plot.hist_area(ax,
                        A1,
                        area=area,
                        c='k',
