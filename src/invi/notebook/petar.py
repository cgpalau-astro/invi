...
def FSR(orb, orb_frw, s_dict, s=0.1):
    #-------------------------------------------------------
    def plots_orb(ax, x_orb, y_orb, x_orb_frw, y_orb_frw, x_label, y_label):
        ax.plot(x_orb, y_orb, color="k", linestyle="-", zorder=0, linewidth=1.0)
        ax.plot(x_orb_frw, y_orb_frw, color="k", linestyle="--", zorder=0, linewidth=1.0)
    ...
    def plots_str(ax, x_str, y_str, s):
        ax.scatter(x_str, y_str, color="r", s=s, zorder=1)
    #-------------------------------------------------------
    ...
    plots_str(ax1, car_str['x'], car_str['y'], s)
    plots_str(ax2, cyl_str['R'], cyl_str['z'], s)
    plots_str(ax3, car_str['x'], car_str['z'], s)
    plots_str(ax4, car_str['v_y'], car_str['v_z'], s)


def ICRS(orb, orb_frw, s_dict, s=0.1):
    ...
    #Plot orbit
    kwargs = {'c': 'k', 'linewidth': 1.0, 'zorder': 0}
    ax.plot(orb_sph['alpha'], orb_sph['delta'], linestyle="-", **kwargs)
    ax.plot(orb_frw_sph['alpha'], orb_frw_sph['delta'], linestyle="--", **kwargs)

    #Plot stars
    ax.scatter(str_sph['alpha'], str_sph['delta'], c="r", s=s, zorder=1)

    #Initial - Final points
    kwargs = {'s': 25.0, 'zorder': 2}
    ax.scatter(orb_sph['alpha'][0], orb_sph['delta'][0], c="b", **kwargs)
    ax.scatter(orb_sph['alpha'][-1], orb_sph['delta'][-1], c="g", **kwargs)
