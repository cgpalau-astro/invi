...
__all__ = [#1-distribution
           "peaks_definition",
           "plot_A1_F1", "plot_A1_A2", "plot_Ai_Fi", "plot_Ji",
           ...
           "plot_fit", "plot_Ar_stripping_Ar", "plot_hist_Fi",
           ]
...
def plot_A1_A2(data, data_ref=None, tips=False, x_limit=1.2, s=0.2, gray='0.8', zorder=0):
    """Example: data = {'A1': A1, 'F1': F1, 'leading': l, 'trailing': t}"""
    ...
    if data_ref is not None:
        l = data_ref['leading']
        t = data_ref['trailing']
        ax.scatter(data_ref['A1'][l], data_ref['A2'][l], s=s, c=gray, zorder=zorder)
        ax.scatter(data_ref['A1'][t], data_ref['A2'][t], s=s, c=gray, zorder=zorder)

    #Plot data
    l = data['leading']
    t = data['trailing']
    ax.scatter(data['A1'][l], data['A2'][l], s=s, c="r", label='leading')
    ax.scatter(data['A1'][t], data['A2'][t], s=s, c="k", label='trailing')
    ax.legend(markerscale=35.0*s, loc='upper left')
...
def plot_Ar_Fi(i, data):
    ...
    match i:
        ...
        case 2:
            ...
            ax.scatter(data['l_stripping_Ar'], data['l_F2']*1_000.0**2.0, s=0.015, c="k")
            ax.scatter(data['t_stripping_Ar'], -data['t_F2']*1_000.0**2.0, s=0.015, c="k")
            y_limit = 20.0
            ax.set_ylim(-y_limit, y_limit)
        case 3:
            ax.set_ylabel("F3 [mrad/Gyr]")
            ax.scatter(data['l_stripping_Ar'], data['l_F3']*1_000.0**2.0, s=0.015, c="k")
            ax.scatter(data['t_stripping_Ar'], -data['t_F3']*1_000.0**2.0, s=0.015, c="k")
            y_limit = 20.0
            ax.set_ylim(-y_limit, y_limit)
...
def plot_Ar_wrap_Fi(i, data):
    ...
    match i:
        ...
        case 2:
            ax.set_ylabel("F2 [mrad/Gyr]")
            ax.scatter(_model_3.wrap(data['l_stripping_Ar']), data['l_F2']*1_000.0**2.0, s=0.015, c="k")
            ax.scatter(_model_3.wrap(data['t_stripping_Ar']), -data['t_F2']*1_000.0**2.0, s=0.015, c="k")
            y_limit = 20.0
            ax.set_ylim(-y_limit, y_limit)
        case 3:
            ax.set_ylabel("F3 [mrad/Gyr]")
            ax.scatter(_model_3.wrap(data['l_stripping_Ar']), data['l_F3']*1_000.0**2.0, s=0.015, c="k")
            ax.scatter(_model_3.wrap(data['t_stripping_Ar']), -data['t_F3']*1_000.0**2.0, s=0.015, c="k")
            y_limit = 20.0
            ax.set_ylim(-y_limit, y_limit)
...
def plot_Ar_wrap_centre_Fi(i, data):
    ...
    match i:
        ...
        case 2:
            ax.set_ylabel("F2 [mrad/Gyr]")
            ax.scatter(transform(data['l_stripping_Ar']), data['l_F2']*1_000.0**2.0, s=0.015, c="k")
            ax.scatter(transform(data['t_stripping_Ar']), -data['t_F2']*1_000.0**2.0, s=0.015, c="k")
            y_limit = 20.0
            ax.set_ylim(-y_limit, y_limit)
        case 3:
            ax.set_ylabel("F3 [mrad/Gyr]")
            ax.scatter(transform(data['l_stripping_Ar']), data['l_F3']*1_000.0**2.0, s=0.015, c="k")
            ax.scatter(transform(data['t_stripping_Ar']), -data['t_F3']*1_000.0**2.0, s=0.015, c="k")
            y_limit = 20.0
            ax.set_ylim(-y_limit, y_limit)
...
def plot_fit(x, y, y_label, x0):
    ...
    #--------------------------------------------------------
    coef = _ssc.fit_parameters(x, y, x0)
    s0 = coef[2]
    s1 = coef[3]
    limit = 2.0*s0/(s0 + s1)
    _fnc.numeric.print_array(coef, precision=15)
...
def plot_hist_Fi(i, data):
    #Select two complete periods
    two_periods = _fnc.numeric.within_equal(data['s_stripping_Ar'], -4.0*_np.pi, 0.0)
    ...
    _fig, ax = _fnc.plot.figure(fc=(1.5, 1.0))

    #Kappa
    x[t] *= -1.0

    x = x[two_periods & st]*fc_units
    ax.hist(x, color="k", bins=80, histtype='step', density=True, range=rng)

    match i:
        case 1:
            #Fit normal distribution
            var = _scipy.stats.lognorm.fit(x)
            lognorm = _scipy.stats.lognorm(*var)
            print(f"lognorm var = {var}")

            #Plot log-normal distribution
            a = _np.linspace(rng[0], rng[1]+5.0, 1_000)
            b = lognorm.pdf(a)
            ax.plot(a, b, c="r")

            #Fit normal distribution
            var = _scipy.stats.norm.fit(x)
            norm = _scipy.stats.norm(*var)
        ...
        case 2 | 3:
            ...
            #Statistical test
            ks_statistic = _fnc.stats.kolmogorov_smirnov.test(x, norm.cdf)
            _fnc.stats.kolmogorov_smirnov.print_result(x, ks_statistic, alpha=0.05)
