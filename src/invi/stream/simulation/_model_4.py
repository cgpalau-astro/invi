...
def F_Ar(Ar_strip, prm_sim, n_stars, rng):
    ...
    lognorm = scipy.stats.lognorm(*prm_sim['model_4']['F1_coef']) #[mrad/Myr]
    F1 = invi.units.milli_to_unit(lognorm.rvs(size=n_stars, random_state=invi.misc.seed(rng))) #[rad/Myr]
...
def proportional_to_Ar(data, kappa, prm_sim, rng):
    ...
    n_stars = number_stripped_stars(prm_sim['model_4']['mass_loss'], data['T'])
    Ar_strip = stripping_Ar(data, prm_sim['model_4']['stripped_stars_coef'], n_stars, rng)
...
def model_4(prm_gc, prm, T, random_state):
    #Parameters
