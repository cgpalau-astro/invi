...
def F_Ar(Ar_strip, prm_sim, n_stars, rng):
    ...
    mean = double_exp_wrap(Ar_strip, prm_sim['model_5']['F1_mean_coef']) #[mrad/Myr]
    std = double_exp_wrap(Ar_strip, prm_sim['model_5']['F1_std_coef']) #[mrad/Myr]
...
def proportional_to_Ar(data, kappa, prm_sim, rng):
    ...
    n_stars = number_stripped_stars(prm_sim['model_5']['mass_loss'], data['T'])
    Ar_strip = stripping_Ar(data, prm_sim['model_5']['stripped_stars_coef'], n_stars, rng)
...
def model_5(prm_gc, prm, T, random_state):
    #Parameters
