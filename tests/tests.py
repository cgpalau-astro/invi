...
Note
----
1)  Run pytest from 'invi' folder: 'python3.11 -m pytest -Wall -vs --durations=0 lib/invi/tests/tests.py'
...
def test_reddening():
    ...
    kwargs = {'dec': dec, 'ra':ra, 'correction': 0.0}
...
def test_cross_match():
    ...
    z0_ecl = invi.cross_match.get_data(z1, matches_ecl, verbose=False)

    #Cross match by id
    matches_id = invi.cross_match.id(id_0, id_1, verbose=False)
    z0_id = invi.cross_match.get_data(z1, matches_id, verbose=False)
